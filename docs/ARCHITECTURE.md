...
Поэтому в момент постановки джобы бот регистрирует **`Delivery`** (`POST /deliveries`):
`job_id`, `chat_id`, `message_id`. Отдельная таблица, а не колонки в `job`, потому что джобы
