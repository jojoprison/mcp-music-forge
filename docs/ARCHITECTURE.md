...
процесса `api` и ходил бы по сети сам к себе. 🛑 Эндпоинтом это выставлять нельзя: `api`
слушает `0.0.0.0:8033` без аутентификации, и параметр `chat_id` позволил бы кому угодно
заставить бота слать файлы чужим людям. Отдельная таблица, а не колонки в `job`, потому что джобы
