...
Две вещи здесь не случайны. Предикат «кого ещё ждут» — это **вызов функции досыла**
(`pending_job_ids`), а не повтор её условия: переписанное своими словами условие уже теряло
слагаемое `job.status == failed`, из-за чего каталоги успешно выданных джоб не чистились
вовсе. И каталог удаляется **целиком**, а не по половинам — `final/` читает досыл,
