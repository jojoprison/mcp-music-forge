...
def register_delivery(
    ...
    `POST /deliveries` — и получалась дыра: api слушал тогда `0.0.0.0:8033`
    без аутентификации, то есть кто угодно мог зарегистрировать доставку на
    чужой `chat_id` и заставить бота слать файлы чужим людям. Порт с тех пор
    закрыт (`API_HOST=127.0.0.1`), но эндпоинт всё равно не нужен: защита не
    должна держаться на том, что снаружи никто не достучится. Эндпоинт не нужен
